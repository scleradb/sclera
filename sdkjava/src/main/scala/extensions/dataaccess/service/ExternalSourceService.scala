...
import com.scleradb.external.service.{ExternalSourceService => ScalaExtService}

/** Abstract base class for all external data services - Java version */
abstract class ExternalSourceService extends ScalaExtService {
    /** Create a source object using the given generic parameters */
    def createSource(
        params: Array[ScalValueBase]
    ): ExternalSource

    /** Create a source object using the given generic parameters (Scala) */
    override def createSource(
        params: List[ScalValueBase]
    ): ExternalSource = createSource(params.toArray)
}
